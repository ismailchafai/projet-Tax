...
export class Taxe38EditAdminComponent implements OnInit {
    ...
    public prepareEdit() {
        this.item.datePresentaion = this.service.format(this.item.datePresentaion);
    }
 ...
 public edit(): void {
   ...
   public addTaxe38Details() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.taxe38DetailsElement.id == null){
                this.item.taxe38Details.push(this.taxe38DetailsElement);
            }else{
                const index = this.item.taxe38Details.findIndex(e => e.id == this.taxe38DetailsElement.id);
                this.item.taxe38Details[index] = this.taxe38DetailsElement;
            }
    ...
    private validateTaxe38DetailsCode(){
   ...
   public async openCreateTrim(trim: string) {
        const isPermistted = await this.roleService.isPermitted('Trim', 'edit');
        if (isPermistted) {
